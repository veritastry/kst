...
int spectraldenoise::CalNoiseSpectral(int is_updata,const float*spectral) {
	...
	if (!(is_updata)) {
		...
		for (int i = 0; i < len; i++) Power += spectral[i];
...
int spectraldenoise::denoise() {
	float *spectral = (float *)malloc(len * sizeof(float));
	float *Speech = (float *)malloc(len * sizeof(float));
	memset(spectral, 0, len * sizeof(float));
	double time = 0;
	double ola = 0.5;
	...
	float*arg = (float *)malloc(len * sizeof(float));
	...
	for (long long k = 0; k < length / len1; k++) {
		...
		for (int i = 0; i < len; i++) {
			in[i].r = ((Speech[i])) * cos(arg[i]);
			in[i].i = ((Speech[i])) * sin(arg[i]);
		}
