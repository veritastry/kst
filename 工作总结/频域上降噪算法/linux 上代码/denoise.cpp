...
ss::ss(const char *fliename) :spectraldenoise(fliename) {
	PrevSpeech = (float *)malloc(2 * len * sizeof(float));
	memset(PrevSpeech, 0, 2 * len * sizeof(float));
}
int ss::CalSpeechSpectral(float* Speech, const float*spectral) {
	...
	memmove(PrevSpeech, PrevSpeech + len, len * sizeof(float));
	memmove(PrevSpeech + len, Speech, len * sizeof(float));
	return 1;
}
ss::~ss() {
	free(PrevSpeech);
